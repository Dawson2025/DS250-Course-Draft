...
# Write a one-sentence takeaway comparing my year with the peak
note_q1 = (
...
age_low, age_high = 2025 - peak_year, 2025 - guess_year
# Mention the years with the lowest usage so we can say which ages we would avoid
early_year = int(brittany_history.loc[brittany_history["Total"] == brittany_history["Total"].min(), "year"].iloc[0])
late_year = int(brittany_history.loc[brittany_history["Total"] == brittany_history["Total"].tail(1).values[0], "year"].iloc[0])
# Write a plain sentence explaining the guess and the ages to avoid
note_q2 = (
    f"Brittany peaked in {peak_year} and averages around {guess_year}, so I would guess she is {age_high} or {age_high + 1}. "
    f"I would avoid guessing ages tied to the very early years like {early_year} or the late-year dip in {late_year} because almost no babies were named Brittany then."
)
```
